...
export class ChatGptBrowserWorker {
  ...
  private async reconcileAssistantTurnBinding(
    ...
    let identity = chatGptReboundTurnIdentity(
      baseline.initialTurnIdentities,
      binding.identity,
      state.responseIdentities,
    );
    ...
    if (newUsers.length > 0) {
      ...
      if (!matches) {
        if (binding.identity.startsWith("group:assistant:")) {
          throw new Error("ChatGPT opened another user turn while the bound assistant response was detached");
        }
        // The older renderer re-keys every turn when a finished Pro response reloads the thread.
        identity = chatGptReconciledTurnIdentity(baseline.initialTurnIdentities, binding, state);
      }
    }
